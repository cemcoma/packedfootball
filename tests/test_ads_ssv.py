...
QUERY = b"ad_network=5450213213286189855&ad_unit=1234567890&custom_data=reward&reward_amount=1&reward_item=coins&timestamp=1758380000000&transaction_id=abc123&user_id=uid42"
...
def test_first_reward_ad_pays_step_zero():
    tx = FakeTx({"users/u": _profile()})
    paid = ads.grant_in_tx(tx, "users/u", "reward", T0)
    step = config.AD_REWARD_PATH[0]
    assert paid["credits_remaining"] == 100 + step["credits"]
    assert paid["reward_ads_watched"] == 1
    assert tx.docs["users/u"]["credits"] == 100 + step["credits"]


def test_reward_track_walks_the_path_then_refuses():
    tx = FakeTx({"users/u": _profile()})
    for _ in config.AD_REWARD_PATH:
        ads.grant_in_tx(tx, "users/u", "reward", T0)
...
def test_counters_reset_on_the_next_game_day():
    tx = FakeTx({"users/u": _profile(reward_ads_watched=3, energy_ads_watched=3)})
    tomorrow = T0 + timedelta(days=1)
    paid = ads.grant_in_tx(tx, "users/u", "reward", tomorrow)
    assert paid["reward_ads_watched"] == 1
    assert tx.docs["users/u"]["energy_ads_watched"] == 0
    assert tx.docs["users/u"]["last_ad_date"] == ads.game_date(tomorrow)


def test_energy_ad_adds_one_point_and_refuses_when_full():
    tx = FakeTx({"users/u": _profile()})
    paid = ads.grant_in_tx(tx, "users/u", "energy", T0)
    assert paid["energy"] == 3 + config.AD_ENERGY_REWARD
    tx = FakeTx({"users/u": _profile(**{energy_service.ENERGY_FIELD: config.ENERGY_MAX})})
    with pytest.raises(ads.AdRewardRefused):
        ads.grant_in_tx(tx, "users/u", "energy", T0)


def test_unknown_track_is_refused():
    tx = FakeTx({"users/u": _profile()})
    with pytest.raises(ads.AdRewardRefused):
        ads.grant_in_tx(tx, "users/u", "bucks", T0)


def test_status_counters_read_zero_after_the_day_rolls():
    profile = _profile(reward_ads_watched=2)
    assert ads.counters(profile, ads.game_date(T0))["reward_ads_watched"] == 2
    assert ads.counters(profile, ads.game_date(T0 + timedelta(days=1)))["reward_ads_watched"] == 0
